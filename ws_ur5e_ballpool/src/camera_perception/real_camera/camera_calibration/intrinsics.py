...
class ChessboardCalibrator(object):
    ...
    def calibrate(self, undistortion_alpha=1.0):
        if len(self.images) > 0:
            ret, self.camera_matrix, self.camera_distortions, rvecs, tvecs = cv2.calibrateCamera(
                self.chessboard_world_points, self.chessboard_image_points, self.gray_images[0].shape[::-1], None, None
            )
    ...
    def refineCameraMatrix(self, alpha=1):
        if len(self.images) > 0:
            ...
            self.camera_matrix_refined, roi = cv2.getOptimalNewCameraMatrix(
                self.camera_matrix, self.camera_distortions, (w, h), alpha, (w, h)
            )

    def buildUndistortedImages(self):
        for img in self.images:
            dst = cv2.undistort(img, self.camera_matrix, self.camera_distortions, None, self.camera_matrix_refined)
    ...
    def run_calibration(self, list_imgs):

        #######################################
        # Feed with images
        #######################################
        self.discarded = []
        for i, img in enumerate(tqdm(list_imgs)):
        ...
        self.calibrate(undistortion_alpha=1.0)
        ...
        print(
            np.array2string(
                self.singleRowPrint(reference_image, self.camera_matrix, self.camera_distortions), separator=","
            )
        )
        ...
        print(
            np.array2string(
                self.singleRowPrint(reference_image, self.camera_matrix_refined, self.camera_distortions),
                separator=",",
            )
        )

        return copy.deepcopy(self.camera_matrix_refined), copy.deepcopy(self.camera_distortions)

    def undistort_and_show(self, list_imgs, camera_matrix, camera_distortion):
...
if __name__ == "__main__":
    ...
    ap.add_argument("--image_folder", default="./ws_ur5e_ballpool/src/camera_perception/real_camera/camera_calibration/calib_data/data_21_09", type=str)
    ...
    img_files = glob.glob(os.path.join(args["image_folder"], "*." + args["image_extension"]))
    list_imgs = [cv2.imread(img_file, cv2.IMREAD_COLOR) for img_file in img_files]

    K, D = chessboard_calibrator.run_calibration(list_imgs)

    if args["save_params"]:
        np.savetxt(os.path.join(args["image_folder"], "camera_matrix.txt"), K)
        np.savetxt(os.path.join(args["image_folder"], "camera_distortion.txt"), D)

    if args["show_undistorted"]:
        chessboard_calibrator.undistort_and_show(list_imgs, K, D)
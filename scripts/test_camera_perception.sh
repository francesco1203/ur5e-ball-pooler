...
use_real_camera="true"                      #true se vuoi usare la camera reale, false se vuoi usare i bag files o il nodo fake_camera_node
launch_driver="true"                         #true se vuoi (ri)lanciare il driver (avviato solo con camera reale)
...
if [ "$use_real_camera" = true ] && [ "$launch_driver" = true ]; then
    ...
    if [ "$use_bagfiles_stream" = true ]; then
        ...
        gnome-terminal --tab --title="BagFile Player" -- bash -c "source ${INSTALL_SETUP_BASH} && ros2 bag play ${bagfile_path} ; exec bash"
        ...
        if [ "$launch_vision_node" = true ]; then
            echo "Avvio Detection e Perception..."
            gnome-terminal --tab --title="VisionNode" -- bash -c "source ${INSTALL_SETUP_BASH} && ros2 launch real_camera vision.launch.py; exec bash"
            sleep 1
        fi
        # ------------------------------------------------
